...
without claiming durability. Unexpected Adapter exceptions log a correlation ID plus safe
operation context and expose only a bounded indeterminate failure with that ID. Optional
...
- `AioSandboxProvider` (`packages/harness/deerflow/community/`) - Docker/Kubernetes isolation. Local Docker supports nonempty accepted snapshots by mounting the selected active view read-only. Remote/provisioner AIO defaults to `empty_only`; it advertises `immutable_read_only` for a sandbox only after an `rwx_verified_copy_v1` request yields an exact materialization receipt. The provisioner mounts one content-addressed snapshot subpath from a cross-node RWX home claim into a verifier init container only, performs bounded symlink-safe copy and canonical digest checks, publishes into a private per-Pod `emptyDir`, and mounts only that copy read-only into the sandbox. Mutable live skill volumes and source aliases are absent. A per-attempt capability protects direct Pod-IP access; it is held only in Gateway process memory and the Lease-owned immutable Kubernetes Secret for that Pod attempt, and never enters RunRow, lifecycle events, checkpoints, or logs. One native Kubernetes Lease is the attempt owner root: the Pod, immutable evidence/capability Secrets, and NetworkPolicy share its UID; the Gateway renews the exact Pod/Lease/materialization tuple only after authoritative RunRow renewal, and a bounded reconciler deletes expired owner roots. Response-loss replay may reuse only the same Lease identity and capability. Gateway management requests use a rotating audience-bound projected ServiceAccount token, validated by TokenReview against the exact Gateway namespace and ServiceAccount; authenticated profile preflight participates in readiness. The Helm profile requires digest-pinned sandbox and verifier/gate images and rejects RWO rather than adding a same-node requirement; provisioner readiness checks the actual claim is `Bound` and reports `ReadWriteMany`. Accepted Pods only prefer a different Gateway node when one is available. Active-cache and warm-pool entries are checked with the backend during acquire/reuse; an accepted Pod cannot be adopted after process loss because its capability is intentionally unavailable, matching orphan terminalization. `get()` remains an in-memory lookup for event-loop-safe tool paths. `uses_thread_data_mounts` defaults to backend detection (`LocalContainerBackend=True`, remote/provisioner backends=False), while `sandbox.thread_data_mounts` may assert shared thread data. Readiness and sandbox clients disable ambient HTTP proxies for control-plane destinations.
...
- `migrations/versions/0011_accepted_invocation.py` — adds nullable host-sealed acceptance facts and pinned agent revision evidence for normal runs
...
`0010_run_cancel_request` through each of 0011–0016, then exercises repository
