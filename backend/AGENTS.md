...
- `deerflow-runtime-api==0.1.0` is the supported host-independent in-process contract. `app/runtime/api.py` binds one authenticated service identity and translates its exact `ensure`, invocation/context `observe`, fenced `control(cancel)`, and `capabilities` records to `InvocationRuntime`; callers cannot provide principal, scope, Origin, accepted digests, revision, config/context/metadata bags, delivery settings, or credentials. The host derives a canonical `service:v1:sha256:...` scope from `["service", authenticated_service_id]`. Responses project only fixed public row/event fields and safe failure codes. The adapter is in-process only—do not infer or add runtime HTTP routes.
