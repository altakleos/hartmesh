...
def test_accepted_snapshot_binding_uploads_only_the_selected_digest(
    ...
    provider._thread_sandboxes[provider._thread_key("thread-1", "user-1")] = "fake-sb-1"
...
def test_unclearable_partial_projection_poisons_remote_sandbox(
    ...
    assert "fake-sb-1" not in provider._sandboxes
    assert provider._thread_key("thread-1", "user-1") not in provider._thread_sandboxes
...
def test_failed_remote_clear_quarantines_accepted_profile_as_success(
    ...
    provider._thread_sandboxes[provider._thread_key("thread-clear", "user-clear")] = "fake-sb-clear"
    ...
    assert provider._thread_key("thread-clear", "user-clear") not in provider._thread_sandboxes
...
def test_dead_accepted_profile_sandbox_finalizes_from_fenced_local_binding() -> None:
    ...
    provider = _make_provider()
    provider._thread_sandboxes[provider._thread_key("thread-dead", "user-dead")] = "fake-sb-dead"
    provider._accepted_skill_bindings = {
    ...
    assert provider._accepted_skill_bindings == {}
    assert provider._thread_key("thread-dead", "user-dead") not in provider._thread_sandboxes
...
def test_failed_prepublication_remote_clear_quarantines_exact_profile(
    ...
    provider._thread_sandboxes[provider._thread_key("thread-unpublished", "user-unpublished")] = "fake-sb-unpublished"
...
def test_missing_prepublication_client_uses_persisted_profile_quarantine() -> None:
    ...
    provider = _make_provider()
    provider._thread_sandboxes[provider._thread_key("thread-missing", "user-missing")] = "fake-sb-missing"
    ...
    assert provider.ensure_accepted_skill_snapshot_absent(clear)
    assert provider._accepted_skill_quarantines == {}
    assert provider._thread_key("thread-missing", "user-missing") not in provider._thread_sandboxes
...
def test_recreated_accepted_sandbox_cannot_inherit_stale_quarantine(
    ...
    provider._thread_sandboxes[provider._thread_key("thread-recreated", "user-recreated")] = sandbox_id
    ...
    def recreate(thread_id, *, user_id, accepted_skills_only):
        ...
        provider._thread_sandboxes[provider._thread_key(thread_id, user_id)] = sandbox_id
        return sandbox_id
...
def test_teardown_refuses_to_kill_an_invocation_owned_projection(
    ...
    p._thread_sandboxes = {p._thread_key(identity[1], identity[0]): "sb-projection-owned"}
