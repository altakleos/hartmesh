...
- **FIFO and bounds**: claims select bounded pages and only the earliest unfinished receipt for each thread. Busy work retries on a fixed, non-tight cadence without spending the poison failure budget; malformed or failing processing uses bounded exponential retry and a finite failure cap. Automatic retention removes completed receipts only. Unresolved dead letters remain available through administrator-only capped state/due-age summary, exact-ID safe inspection, and exact-fence requeue operations; requeue requires the provider-event digest (or explicit legacy null), returns the row to `deferred`, and emits only pseudonymous administrator/correlation audit evidence. No list or bulk mutation surface exists.
