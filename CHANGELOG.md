...
- hartmesh#132 — membership follows the claim the provider already emits, so a company that runs one identity provider across many tenants stops giving everyone an account everywhere. One named claim with admitting values decides admission, before an account is created and before a linked one is returned; the person sees a message they can act on that does not name the claim, and the log carries the issuer and subject, never a token. A value-to-role mapping rewrites the role at every sign-in in both directions, so an administrator appointed after their first sign-in becomes one and a demoted one stops being one. A deployer command, not a route, disables an account by issuer and subject, ends its sessions, revokes its tokens and refuses its scheduled work; a run already executing finishes under its own budget and its stream ends with it, which is stated rather than left to chance. Disabling a subject who has never signed in is recorded, so a person removed before their first day cannot make an account later.

- hartmesh#133 — sharing the same file twice lands once. *Share with everyone* used to disable itself after a click, but only in the page's own memory: opening the Shared tab and coming back re-enabled it, and a second tab or a later day never knew at all, so a company's Shared area filled with `report_1.pdf` and `report_2.pdf`. The page keeps no such memory now. The publish route digests the file and answers with the copy already holding those bytes in that folder, so a second click from anywhere, a colleague's identical file, and a retried request all reach one entry; **Undo** takes back only what that click put there. Every live record of those bytes is checked against the disk, so a file an operator removed or replaced by hand neither blocks a fresh copy nor makes every later share add one.

- hartmesh#134 — a report shared by a colleague reading Chinese and the same report shared by a colleague reading English land in the same folder. The folder name came from the reader's language, which split the company's reports in two and made each half invisible from the other, and defeated the same-bytes check above, which is decided per folder. A folder name in a shared directory is data, not words on one person's screen. Reports already filed under the translated name stay where they are; their publisher or an administrator moves or removes them.

- hartmesh#135 — where a shared file lands is decided by the file, in one place, for all three ways of sharing. The report card filed a report's downloads under *Reports* while the artifact panel and a row of *My files* dropped the same file at the root, so one report reached Shared twice. A report's download is now filed with the reports wherever it is shared from, recognised by the report it belongs to rather than by its name, and keeping one in *My files* files it the same way, so a report kept and later shared lands beside the copy the card would have shared. The rest of how someone keeps their own files does not cross into the company's area: a folder named for a customer or a deal would otherwise become a company folder on one click, and two people who file one report differently would put two copies in Shared. Both confirmations name the folder, because the person never chose it.
