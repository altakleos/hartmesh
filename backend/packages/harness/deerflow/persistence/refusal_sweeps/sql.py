...
class RefusalSweepRepository:
    ...
    async def register(self, process_id: str) -> int:
        ...
        async with self._sf() as session:
            ...
            await session.execute(insert(GatewayProcessRow).values(process_id=process_id, started_at=now, heartbeat_at=now, checked_through=latest))
            await session.commit()
            return latest

    async def beat(self, process_id: str, *, checked_through: int | None = None) -> None:
        async with self._sf() as session:
            now = await self._now(session)
            ...
            if not result.rowcount:
                ...
                await session.execute(insert(GatewayProcessRow).values(process_id=process_id, started_at=now, heartbeat_at=now, checked_through=checked_through or 0))
            await session.commit()
    ...
    async def live_processes(self, *, window_seconds: float) -> list[LiveProcess]:
        ...
        async with self._sf() as session:
            ...
            return [LiveProcess(row.process_id, int(row.checked_through)) for row in rows if coerce_database_wall_clock(row.heartbeat_at) >= cutoff]

    async def record_endings(self, process_id: str, check_id: int, endings: dict[str, dict[str, int]]) -> None:
        rows = [{"process_id": process_id, "user_id": user_id, "surface": surface, "count": count, "check_id": check_id} for user_id, surfaces in endings.items() for surface, count in surfaces.items() if count]
        if not rows:
            return
        async with self._sf() as session:
            now = coerce_database_wall_clock(await session.scalar(select(await self._now(session))))
    ...
    async def endings_for(self, user_ids: list[str], *, since_check: int) -> list[SurfaceEnding]:
        ...
        async with self._sf() as session:
            ...
            return [SurfaceEnding(row.process_id, row.user_id, row.surface, int(row.count), coerce_database_wall_clock(row.ended_at)) for row in rows]
