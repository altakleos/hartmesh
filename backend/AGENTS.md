...
- Admission also seals the effective `ExecutionBudgetV1`. The worker restores
  its compact `ExecutionPolicyStateV1` before model work and advances it with an
  owner/epoch/lease-fenced compare-and-swap; accepted durable runs fail closed
  on missing keys, drift, malformed state, or a stale writer. Tool equivalence
  uses startup-frozen secret HMAC keys, while `policy.decision.v1` and the
  authorized evidence summary expose only safe counters, stable reason codes,
  qualification state, and public references. See
