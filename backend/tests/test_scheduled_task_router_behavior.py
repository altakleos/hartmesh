...
@pytest.mark.asyncio
async def test_pause_audit_failure_prevents_scheduled_task_mutation(monkeypatch):
    repo = _Repo()
    task = await repo.create(
        task_id="task-audit",
        user_id="user-1",
    ...
    assert (await repo.get(task["id"], user_id="user-1"))["status"] == "enabled"
