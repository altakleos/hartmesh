...
HartMesh's `/api/runtime/v1/*` and in-process adapters share one
`InvocationRuntime`; do not bypass its accepted identity/material admission.
`GracefulShutdownCoordinator` exclusively orders producer stop, run drain,
memory flush, and dependency close. If quiescence is unproven, retain resources
for process reclamation rather than closing them under active writers.
...
CORS defaults to same-origin through nginx on port 2026. Split-origin or
port-forwarded clients must set exact `GATEWAY_CORS_ORIGINS`, shared by CORS and
CSRF middleware, and expose `Content-Location` via `CORS_EXPOSED_HEADERS`. The
LangGraph SDK reads new run IDs from that non-safelisted header; without it,
`useStream.onCreated` does not fire, placeholder routes persist, and edit,
regenerate, and branch stay hidden until reload. Same-origin clients are unaffected.
...
- `runs.operation_kind` makes user runs, `checkpoint_write`, and `artifact_write` first-class operations under one durable active-thread uniqueness constraint. New kinds use `create_thread_operation_atomic()` and `reserve_thread_operation()`. Live or lease-less reservations are non-interruptible; expired leased reservations may be reclaimed immediately. Lease-less failures stay fail-closed and heartbeat-disabled multi-worker mode is unsupported. The reservation context attaches the caller task immediately after admission, converts detected lease loss to retryable `ConflictError`, revalidates failed renewals under the manager lock, and always performs local cleanup. Reservations stay out of run history and run-only helpers; release uses the captured owner. `create_run_atomic()` is a deprecated normal-run compatibility shim.
