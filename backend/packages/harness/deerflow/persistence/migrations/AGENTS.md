...
- `migrations/versions/0032_subagent_batch_evidence.py` — current head; upgrades durable subagent batches to tenant-scoped writer-v2 acceptance/execution commitments, immutable item digests, lease epochs, cancellation fences, and append-only attempt evidence. Existing rows remain explicit `legacy_unbound`; downgrade blocks after any writer-v2 batch or attempt exists.
