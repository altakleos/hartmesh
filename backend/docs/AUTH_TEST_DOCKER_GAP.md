...
| TC-DOCKER-03 | Per-worker lockout divergence | Confirms the default `auth.local.lockout_store: memory` counters do not share state across `gunicorn` workers (4 by default in the compose file), so an account gets roughly N x the attempts and an admin unlock reaches only the worker that served it. Setting `lockout_store: redis` closes both; the divergence is the documented cost of the in-process default | needs multi-worker container |
