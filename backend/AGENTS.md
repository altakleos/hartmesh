...
| **Durable Runtime** (`/api/runtime/v1`) | Admin-only `GET /capabilities`; authenticated `POST /invocations/ensure`; owner/admin-visible `GET /invocations/{rid}` and `GET /contexts/{thread_id}/invocations`; fenced `POST /invocations/{rid}/control`. This transport uses the strict `deerflow.runtime/v1` records and one versioned `runtime.error` envelope. |
