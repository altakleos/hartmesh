...
- `migrations/versions/0011_accepted_invocation.py` through `0019_inbound_event_identity.py` — HartMesh's durable-invocation branch from `0011_mcp_tasks`
...
- `migrations/versions/0024_tool_receipt_idempotency.py` — adds nullable `run_events.idempotency_key` and the receipt append uniqueness index on `(run_id, event_type, idempotency_key)`
- `migrations/versions/0025_server_owned_tenant_identity.py` — adds nullable safe tenant anchors to runs/events and the one-row `hartmesh_deployment_identity` schema binding. Empty schemas bind atomically at Gateway startup; a nonempty legacy schema may be bound only by the explicit `deerflow deployment bind-tenant --expected-nonempty-schema` operator flow documented in `backend/docs/TENANT_IDENTITY.md`.
- `migrations/versions/0026_mcp_task_lineage.py` — adds the MCP-task schema-writer marker, tenant and immutable lineage columns, parent run/receipt joins, separate cancellation actor/reason fields, tenant-scoped uniqueness, and bounded tenant/thread/due/parent/notification indexes. The task row and lineage are one insert/transaction. Legacy rows retain nullable lineage and decode as `legacy_unavailable`. Before any v2 row exists, downgrade leaves additive columns/tenant uniqueness, restores the predecessor user/server/remote-task uniqueness rule, and removes Project 05 checks/indexes; after a v2 row exists it fails `mcp_task_schema_writer_rollback_blocked`, leaving the unknown 0026 revision to stop an older binary at startup.
- `migrations/versions/0027_multi_gateway_topology.py` — adds the tenant/profile/replica topology registry and the persisted scheduler generation used by deterministic occurrence identity.
- `migrations/versions/0028_mcp_request_commitment.py` — adds nullable private MCP exact-request commitment fields, requires them for writer-v3 rows, removes the four superseded pre-tenant MCP indexes, and reasserts `scheduled_tasks.schedule_version` through the drift-aware helper. Its downgrade restores the exact 0027 index shape only before any v3 row exists.
- `migrations/versions/0029_run_recovery_policy.py` — adds immutable run recovery policy, bounded recovery payloads, database-ordered admission cursors, and the nullable terminal-projection owner/active-epoch proof retained after live ownership is released. Snapshot repair must compare and preserve the original recovery payload. Downgrade is permitted only before exact-two, admission-cursor, or terminal-projection semantics have been used. In `durable_two_gateway_v1`, a Helm pre-install/pre-upgrade Job uses the exact Gateway image and PostgreSQL advisory lock to migrate. Gateway pods call the read-only head verifier and must never fall back to concurrent startup migration.
- `migrations/versions/0030_run_delivery_owner_backfill.py` — repairs only legacy `run.delivery` rows whose owner is NULL and whose run ID, thread ID, and NULL-safe tenant digest select exactly one non-NULL authoritative run owner. Downgrade is deliberately data-preserving because the repaired row's former NULL state cannot be reconstructed safely.
- `migrations/versions/0031_merge_upstream_0017.py` — no-op merge of `0030_run_delivery_owner_backfill` and upstream `0017_personal_access_tokens`
- `migrations/versions/0032_subagent_batch_evidence.py` — upgrades durable subagent batches to tenant-scoped writer-v2 acceptance/execution commitments, immutable item digests, lease epochs, cancellation fences, and append-only attempt evidence. Existing rows remain explicit `legacy_unbound`; downgrade blocks after any writer-v2 batch or attempt exists.
- `migrations/versions/0033_automation_identities.py` — adds nullable legacy tenant anchors plus tenant-first indexes to PATs, backfills only from the already-bound deployment singleton, then makes both anchors `NOT NULL` and creates bounded secret-free credential audit aggregates. A populated unbound PAT table fails before DDL with `credential_tenant_binding_required`; `deerflow deployment bind-tenant --expected-nonempty-schema` catches that precondition, commits the explicit singleton binding, and reruns normal bootstrap. Downgrade blocks once a PAT is bound or any credential audit evidence exists.
- `migrations/versions/0034_tool_plane_revisions.py` — adds tenant-bound scope pointers/generations, immutable skill/MCP revisions, append-only transition events, and base/overlay compatibility attestations. A complete pre-created shape is accepted for hybrid bootstrap, a partial shape fails, and downgrade blocks after any governed row exists.
- `migrations/versions/0035_batch_sandbox_evidence.py` — adds nullable accepted-material request/evidence and bounded sandbox-lifecycle payloads to the existing durable batch-attempt row. Evidence is joined only through the current item/attempt fence; downgrade blocks after any attachment is present.
- `migrations/versions/0036_execution_policy_state.py` — adds the nullable
