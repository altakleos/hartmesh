...
- `migrations/versions/0033_automation_identities.py` — current head; adds nullable legacy tenant anchors plus tenant-first indexes to PATs, backfills only from the already-bound deployment singleton, and creates bounded secret-free credential audit aggregates. The explicit tenant-binding operator flow also binds legacy PAT rows. Downgrade blocks once a PAT is bound or any credential audit evidence exists.
