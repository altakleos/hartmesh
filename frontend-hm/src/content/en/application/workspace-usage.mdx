...
Plan mode comes with **Ultra** mode in the input bar. In plan mode, the agent creates and maintains a visible task list as it works through a complex multi-step objective. Each task shows its status (`pending`, `in_progress`, `completed`) in real time.

Plan mode is most useful for complex tasks with 3 or more distinct steps, which is why Ultra — the mode that also divides work between subagents — is the one that enables it. Flash, Reasoning and Pro run without a task list.
