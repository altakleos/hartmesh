...
| Auditable credential evidence | Session, PAT, internal-service, and channel adapters bind one secret-free credential method/reference and canonical effective-authority digest to the existing identity/Origin/tenant context. PAT routes are explicit and current auth is rechecked; required admission/control audit failure closes the action. | `backend/packages/extension-api/deerflow_extension_api/credentials.py`; `backend/app/gateway/credential_evidence.py`; `backend/packages/harness/deerflow/persistence/credential_audit`; `backend/packages/harness/deerflow/persistence/migrations/versions/0033_automation_identities.py` | `test_trusted_run_context_v4_binds_credential_and_reads_legacy_contexts`; `test_pat_acceptance_is_user_credential_not_acting_service`; `test_revoked_pat_cannot_submit_observe_control_or_export`; `test_conditional_cancel_paths_also_fail_closed_when_audit_is_unavailable` | PostgreSQL migration/backfill contract plus SQLite auth/runtime tests | Implemented; evidence is historical attribution, never current authority |
...
| Accepted execution circuit breakers | Every new durable run binds one canonical caller-narrowable budget and recovers its compact counter/decision state through the owner/epoch/lease fence. Repeated-tool equality is a private startup-keyed HMAC over versioned value-preserving normalizers; public evidence receives only bounded counters and reason codes. | `backend/packages/harness/deerflow/runtime/execution_policy.py`; `backend/packages/harness/deerflow/runtime/runs/worker.py`; `backend/packages/harness/deerflow/persistence/run/sql.py`; `backend/packages/harness/deerflow/persistence/migrations/versions/0036_execution_policy_state.py`; `backend/app/gateway/routers/thread_runs.py` | `test_policy_evaluator_is_pure_and_emits_one_threshold_decision`; `test_policy_state_cas_is_fenced_and_survives_reload`; `test_evidence_summary_endpoint_authorizes_and_projects_legacy_safely`; `test_fingerprint_detects_execution_policy_keyring_skew` | Memory/SQLite concurrency and contract tests; no passing external PostgreSQL/exact-two policy artifact in this checkout | Implemented offline; shared-store and exact-two qualification remain unverified |
...
identity, safe tenant reference, server-created credential evidence, final Origin including contributor references, thread/external-key binding,
...
Current writers use trusted-context serialization v4. Its
`CredentialEvidenceV1` records the boundary method, optional safe credential
reference, and versioned canonical effective-authority digest/categories. A
PAT uses its existing UUID4 as the reference and never creates an acting
service. Caller credential-looking context is scrubbed. Versions 1 through 3
remain readable with no invented credential facts. Revocation leaves v4
history untouched while the authentication middleware blocks the revoked PAT
before any equal replay, observe, control, or export path. Exact-two recovery
keeps that original actor evidence and uses its separately fenced recovery
owner as executor; recovery never rewrites the admission actor. The complete
schema and policy are documented in
[`AUDITABLE_AUTOMATION_IDENTITIES.md`](../../docs/AUDITABLE_AUTOMATION_IDENTITIES.md).
...
`0033_automation_identities` head plus accepted/idempotency/caller-intent/
assembly/tenant and MCP-task lineage/request-commitment columns, recovery-policy
and admission-cursor constraints, tenant-bound PAT anchors, credential-audit
checks/indexes, the schema-identity singleton, the nullable
