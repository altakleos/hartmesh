...
- **A role limit caps what the claim grants** — `python -m app.gateway.auth.accounts limit-role` holds an identity (issuer and subject, account or not) at `user`. A sign-in stores the lower of the role its claim or `admin_emails` grants and the limit, inside the write that stores the role, and every read derives the role from the limit, so a provider restored from a backup whose claim says `admin` again cannot hand the role back. `lift-role-limit` leaves the stored role where the limit held it until a sign-in reads the role again: the next sign-in where roles follow the claim, and under `admin_emails` only a sign-in that changes the account's address.
