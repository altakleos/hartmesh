...
The first full-document load, reload, or compatibility save for a scope removes only that `memory.json` target's interrupted atomic-write siblings, immediately after taking the cross-process scope lock.
Active writers hold the same lock, so simultaneous openers serialize the idempotent cleanup.
...
Honcho is HartMesh's portable remote adapter and the host injects the startup-frozen `TenantIdentityV1.namespace(HONCHO)` into its reserved `_hartmesh_tenant` config key; the portable folder must not import host tenant types.
`HonchoIdentityResolver` derives every bounded workspace, peer, and session from that pseudonymous tenant namespace plus a collision-resistant user/thread component.
Production overrides cannot escape or share the derived workspace; shared search visibility exists only in warned `allow_local_shared_workspaces` local mode.
The host-injected key is removed from file/API config before construction and cannot be caller-selected.
Honcho supplies mutable contextual memory; it is never HartMesh's source of truth for admission, checkpoints, invocation status, authorization, or audit evidence.
Health, readiness, and deployment surfaces report Honcho as an optional mutable-context dependency and do not probe it or expose its endpoint.
...
Goal continuations are deduplicated to one `context:memory` event per run.
Honcho's mutable provider reads use separate `memory.observation.v1` events, never `context:memory`.
Accepted durable runs bind the host observation callback only around graph execution; each completed Honcho operation admitted through that trusted boundary persists one event with tenant reference, hashed workspace, operation/status, exact bounded read-projection digest when applicable, count, truncation, and time, never content, query, raw identity, or provider failure text.
Timed-out candidate reads are discarded before admission and emit no event; a successful admitted read whose persistence fails is discarded under fail-open and raises under fail-closed.
Retries are intentionally not deduplicated and do not claim deterministic replay; the owner-authorized run-events route is the only lifecycle projection.
