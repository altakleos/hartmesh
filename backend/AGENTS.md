...
- **Native-channel trust sequence**: the provider adapter authenticates the platform event before publishing an `InboundMessage`; `ChannelManager` then looks up the connection again by provider identity and revalidates both the active `connection_id` and DeerFlow `owner_user_id` before creating a thread or launch intent. Only after that check does it resolve the agent/thread and construct `InternalNativeChannelFacts`. The raw platform sender remains `channel_user_id`, separate from the authenticated DeerFlow owner. A top-level manager-owned `agent_name` may select a configured route (for example GitHub fan-out), while similarly named fields inside the raw provider payload cannot select an agent.
- **Stable provider ID rule**: native-channel facts carry the first non-empty server-stable `event_id`, `message_id`, or `msg_id` from manager-owned metadata (then the same keys from the raw provider envelope). When a provider supplies none, `provider_message_id` is `None`; never synthesize a content hash or client-generated fallback. The manager/Postgres TTL dedupe store uses the same stable-ID extraction as a bounded fast-path optimization. When present, the trusted ID is also normalized as the runtime external key, so a redelivery that bypasses or outlives TTL dedupe still converges at `RunStore.ensure_run_atomic()`; this durable guarantee lasts with the retained `RunRow`.
