...
- `migrations/versions/0030_run_delivery_owner_backfill.py` — current head; repairs only legacy `run.delivery` rows whose owner is NULL and whose run ID, thread ID, and NULL-safe tenant digest select exactly one non-NULL authoritative run owner. Downgrade is deliberately data-preserving because the repaired row's former NULL state cannot be reconstructed safely.
