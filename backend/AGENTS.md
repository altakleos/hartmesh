...
- **Runtime API** (`packages/runtime-api/deerflow_runtime_api/`): Publishable, standard-library-only embedded API contracts (`deerflow-runtime-api`). It owns strict frozen `deerflow.runtime/v1` DTOs only and must not import `app`, `deerflow`, FastAPI, SQLAlchemy, or `deerflow-extension-api`.
