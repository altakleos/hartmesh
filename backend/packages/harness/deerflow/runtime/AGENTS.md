...
`turn_phases.py`: one in-memory journal per run, monotonic offsets, opened by `run_agent`, found by run id from the SSE consumer; first text means visible assistant text; see its docstring. The launch's own steps (every entry point stamps the intent; `InvocationRuntime.launch` times them) ride the created record as transient `launch_timings`; the worker hands them to the journal, which renders `launch=` ahead of `phases=`. `observe()` hears each phase as it begins, on the opening thread; `turn_progress.py` turns three of them into one advisory `turn_progress` stream frame each (at most once per run, in order; `call_soon_threadsafe` to the run's loop; closed by the worker at run end; failures logged and dropped). `tool_execution` marks where the turn's tool work began, and `tool_calls`/`tool_ms` beside `model_calls`/`model_ms` -- with the per-tool breakdown `tools=14/308000ms(execute_command=9/270000ms,...)`, largest first, capped at `MAX_TRACKED_TOOL_NAMES` distinct names with the rest pooled under `other` -- split what follows the first answer -- a report turn is minutes of commands after seconds of talking, and all of it used to be residual. They are occupancy, not summed durations: overlap counts once, so each figure is inside `total_ms` while their sum need not be -- `busy_ms` merges both and is the one to subtract. A call is paired and closed by the framework's own run id, and one whose end was never seen (a cancelled async tool: `CancelledError` is a `BaseException`, which LangChain's tool base does not catch) is reported as `tools_open=N/Xms` instead of being folded in -- counting depth alone let one lost event report the whole rest of the turn as tool time. A subagent is invoked with its own `callbacks` list, which replaces the inherited one, so its calls are not counted here: a blocking delegation shows up as the one long tool call it is, a backgrounded one only as the call that launched it. LangChain gates every tool callback on `ignore_agent`, which is why `TurnPhaseCallbackHandler` sets it `False`. A tool's registered name is recorded so the time can be attributed; its arguments and output never are. That admits one thing deliberately: MCP and skill tools are named by operator configuration, so those names now reach Gateway logs. `skill_materialization` is attributed by the four spans nested inside it (`accepted_authorization`, `accepted_material_verify`, `skill_projection`, `skill_snapshot_bind`), so a warm turn's pre-model wait names the step it was spent in. On the released local-Docker profile `skill_snapshot.py`'s `bind_skill_snapshot_active_view` runs three times before a warm turn's first model request (provider, worker, sandbox middleware) and once more per sandbox tool call; it verifies the published view in place -- one entry, exact published modes, no empty or unlistable directory, bytes re-digested against the evidence -- and stages a fresh fsync'd copy only when that fails (`docs/ACCEPTED_SANDBOX_EXECUTION.md`, "Attributing the accepted preparation").
...
- `runtime/checkpoint_cache/` + `runtime/checkpointer/cached_saver.py` — delta-mode checkpoint history cache; checkpoint state reads MUST go through `CheckpointStateAccessor`, and the checkpointer may be a `CachedHistorySaver` wrapper — never rely on concrete saver types. Redis ACLs may deny the lifecycle-only `SCAN` purge: the store warns once, disables later purge attempts, and relies on entry TTL instead; other Redis errors remain per-call warnings.
