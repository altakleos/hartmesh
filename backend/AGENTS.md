...
Durable invocation operation authorization is a separate complete opt-in under startup-only operator `config.yaml -> authorization.invocation_operations`; all three flags default false. When enabled, `InvocationRuntime` uses the same resolver-owned provider instance as the other Gateway authorization paths. Start runs after accepted principal/Origin/context, request digest, and pinned agent revision resolution but before admission; deny/indeterminate creates no `RunRow`, lifecycle evidence, worker, graph, or model work. Known keyed rows bypass start, receive a fresh visible-run observe decision, and compare against stored accepted evidence; race losers repeat visibility/observe/digest comparison and never attach. Observe applies current owner/route visibility before `invocation:observe` (`run:<id>` or one `context:<thread-id>` decision per feed), and cancel applies visibility plus `invocation:cancel` before its receipt/CAS mutation. Unknown/invisible rows never call policy; observe/cancel decisions are not cached. Allowed starts persist only bounded authorization generation, policy ID, reason codes, and an evidence digest in existing `decision_evidence_json`. Provider metadata/messages/claims are never persisted. Timeout/exception/malformed/unavailable is `indeterminate` and fails closed. Tests: `tests/test_invocation_start_authorization.py` plus idempotency and lifecycle transition suites.
