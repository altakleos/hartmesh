...
- Descriptor construction remains conditional. It runs when an assembly observer
  ...
  only when observers exist. Keep every descriptor field explicitly classified by
  `runtime/assembly_evidence.py`, and keep skill-catalog canonicalization shared with
  `assembly_descriptor.skill_catalog_digest()`.
