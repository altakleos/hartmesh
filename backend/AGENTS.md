...
- **Runtime API** (`packages/runtime-api/deerflow_runtime_api/`): Publishable, standard-library-only durable invocation contracts (`deerflow-runtime-api`). It owns the `DurableInvocationPort` Protocol and strict, transitively immutable `deerflow.runtime/v1` DTOs only; parsing freezes caller-owned JSON graphs and serialization returns fresh mutable wire copies. It must not import `app`, `deerflow`, FastAPI, SQLAlchemy, or `deerflow-extension-api`.
...
Authorization identity plumbing is independent of whether authorization enforcement is enabled. For durable invocations, Gateway removes caller-supplied split identity, Origin, `is_internal`, `authz_attributes`, and `channel_user_id`; only the separately authenticated internal channel adapter may provide the raw sender ID, and it never confers subject authority. The accepted `InvocationIdentityV1` distinguishes one effective human/service subject from an optional acting service, while sealed Origin owns transport/source facts. `build_principal_from_context` is the shared Principal builder for assembly-time authorization and `GuardrailAuthorizationAdapter`; when accepted identity is present it is authoritative, and the deprecated compatibility `is_internal` view is true only for a service effective subject. Without split identity, an attributed channel user or a non-service/non-internal role is conservatively non-internal. Principal/identity attributes are defensively frozen. When route authorization is enabled, Gateway derives this same split identity and revalidates an internally represented owner before consulting the provider. The built-in RBAC provider validates `authorization.default_role` during provider resolution so an unknown fallback role fails agent construction instead of degrading into an empty tool set. Task delegation carries the accepted identity and Origin plus copied legacy attributes and the run-bound provider through `SubagentExecutor`; `GuardrailMiddleware`, required MCP preparation, and nested subagents receive those same sealed records. Gateway's `AuthorizationProviderResolver` is application-lifecycle owned: one immutable resolution snapshot/generation supplies the exact same provider object to route/resource checks, lead model and tool filtering, execution middleware, and delegated subagents. Legacy `authorization.provider.use` changes atomically replace that snapshot; an extension-contributed factory is constructed once at startup. The synchronous `DeerFlowClient` remains outside this durable Gateway boundary and keeps legacy resolution. Framework-provided `describe_skill` and memory tools are included in Layer 1 but restored to their legacy post-`tool_search` ordering afterward. `DeerFlowClient.stream()` includes its complete legacy Principal in the agent cache key and deep-copies nested attributes so caller mutation cannot make a stale tool set look current.

Gateway route authorization uses `authz.py::resolve_route_permissions()` as the single route integration point for both `AuthMiddleware` and decorator-only authentication. It asks the application-owned resolver for the current immutable snapshot instead of constructing or caching a private provider. When enabled, it evaluates the six registered `threads:*` / `runs:*` permissions as `resource="route"` requests whose targets are the full `resource:action` strings. Decisions use the async provider API and are cached for the request in `AuthContext`; decorators do not call the provider again. Provider resolution or decision errors follow `authorization.fail_closed`, scoped per permission for decision errors. When authorization is disabled, the legacy complete permission set is returned without resolving a provider. Existing `owner_check` enforcement and `require_admin_user()` management gates remain independent and unchanged. Tests: `tests/test_capability_host_authorization.py`, `tests/test_authorization_route_permissions.py`, `tests/test_auth.py`, and `tests/test_auth_middleware.py`.

Durable invocation operation authorization is a separate complete opt-in under startup-only operator `config.yaml -> authorization.invocation_operations`; all three flags default false. When enabled, `InvocationRuntime` uses the same resolver-owned provider instance as the other Gateway authorization paths. Start runs after accepted principal/Origin/context, request digest, and pinned agent revision resolution but before admission; deny/indeterminate creates no `RunRow`, lifecycle evidence, worker, graph, or model work. Known keyed rows bypass start, receive a fresh visible-run observe decision, and compare against stored accepted evidence; race losers repeat visibility/observe/digest comparison and never attach. Observe applies current owner/admin visibility before `invocation:observe` (`run:<id>` or one `context:<thread-id>` decision per feed). The one exception is an operator-established `authorization.service_observation_grants` entry for an exactly authenticated service: `app.runtime.visibility` snapshots at most 128 OR-composed run/thread/owner/source selectors, the SQL or memory store applies that finite predicate before paging, and only then does the coherent provider make its current binary observe decision. Grants require `authorization.enabled` plus `observe_enabled`, are re-read on every request for immediate revocation, cannot be minted from role/internal/prevalidated/request facts, and never apply to cancel. Cancel remains owner/admin-scoped plus `invocation:cancel` before its receipt/CAS mutation. Unknown/invisible rows never call policy; observe/cancel decisions are not cached. Allowed starts persist only bounded authorization generation, policy ID, reason codes, and an evidence digest in existing `decision_evidence_json`. Provider metadata/messages/claims are never persisted. Timeout/exception/malformed/unavailable is `indeterminate` and fails closed. Tests: `tests/test_invocation_start_authorization.py`, `tests/test_service_observation_grants.py`, plus idempotency and lifecycle transition suites.
...
trusted run-context, identifier-domain, and health-probe contracts as of extension API 0.10.0; the old
...
supported one-replica profile. A restart creates a later process generation; Hartmesh does
not coordinate simultaneous generations or rolling replicas.
...
evidence. Required authorization, contributor, constraints, and MCP instances are rejected at
startup unless their authoritative operation is genuinely async; a decorated method is accepted
only when the declared callable is an async function, while health probes retain their separate
documented sync/async compatibility. SQL lifecycle writers transactionally maintain an
independent `retained_count` beside the cursor/pruning singleton. Readiness uses bounded
singleton plus first/last edge reads and verifies that count against the retained cursor range,
so an interior deletion fails without scanning historical events. Admin-only
...
is a separate boolean. The startup-only `deployment.profile: durable_production` rejects
process-local state and participates in readiness, while `local_development` permits it
without claiming durability. Unexpected Adapter exceptions log only a stable code, exception
class, correlation ID, and safe operation context, and expose only a bounded indeterminate
failure with that ID. Raw exception messages and tracebacks are neither public nor retained in
extension diagnostics/log records. Optional
...
`constraint_expired_before_start`. A single invocation-scoped, concurrency-safe dispatch
ledger is shared through lead and delegated subagent contexts. The first canonical
dispatch-ID/intent pair is `NEW`, consumes one physical-start slot, and publishes its immutable
result; an identical in-flight or completed retry is `REPLAY` and reuses that result without a
second executor. Reusing the ID with changed prompt/agent/options/revision material is
`CONFLICT`; a new ID beyond the ceiling is `EXHAUSTED`. Cancellation closes the ledger and
wakes waiters, and process loss discards it with the terminalized invocation. Token-budget middleware remains a
...
Gateway `create_app()` loads plugins once, stores the immutable Capability Host snapshot
and generation on `app.state` and in the process-wide singleton, constructs the
application-owned authorization resolver, and installs one canonical live diagnostics
list. The extension snapshot/generation is startup-immutable; the resolver publishes its
own independent immutable authorization snapshot/generation for atomic legacy config
replacement. Changing `plugins` requires a restart. Later extension contribution points must be added
...
Infrastructure fields are **restart-required**. The authoritative list lives in `packages/harness/deerflow/config/reload_boundary.py::STARTUP_ONLY_FIELDS` and is mirrored by the standardised `"startup-only:"` prefix on the corresponding `Field(description=...)` in `AppConfig`, so IDE hover on those fields surfaces the reason inline (no need to context-switch into this table). Currently registered: `plugins`, `required_capabilities`, `database`, `deployment`, `checkpointer`, `run_events`, `agent_storage`, `stream_bridge`, `sandbox`, `log_level`, `logging`, `channels`, `channel_connections`, `scheduler`, `mcp_tasks`, `run_ownership`, and `dedupe_storage`. Adding a new restart-required field requires updating the registry; drift is pinned by `tests/test_reload_boundary.py`.
...
| **Durable Runtime** (`/api/runtime/v1`) | Admin-only strict portable `GET /capabilities`; admin-only deployment-specific `GET /deployment`; authenticated `POST /invocations/ensure`; owner/admin-visible `GET /invocations/{rid}` and `GET /contexts/{thread_id}/invocations`; fenced `POST /invocations/{rid}/control`. Portable operations use exact `deerflow.runtime/v1` records and one versioned `runtime.error` envelope; deployment facts use the separate `deerflow.deployment/v1` report. |
| **GitHub Webhooks** (`/api/webhooks/github`) | `POST /` - receive GitHub App / repo webhook deliveries. One host-owned authentication classifier verifies `X-Hub-Signature-256` against `GITHUB_WEBHOOK_SECRET` and supplies the same trust fact to route mounting, startup validation, readiness, and the deployment report; the route is exempt from auth + CSRF because authenticity is enforced by HMAC. It is mounted only when a secret is set, or when `local_development` explicitly enables `DEER_FLOW_ALLOW_UNVERIFIED_GITHUB_WEBHOOKS=1`. Unverified mode is always `best_effort`, cannot satisfy `durable_production`, and a durable process whose secret disappears returns 503 rather than falling through to it. Recognized events include `ping`, `issues`, `issue_comment`, `pull_request`, `pull_request_review`, and `pull_request_review_comment`; unknown events return 200 with `handled=false`. Fan-out runtime failures return 503, keeping the delivery recorded as failed for manual/API/scripted redelivery (GitHub does not automatically retry any failed delivery, 5xx included); permanent/non-retryable conditions such as `channels.github.enabled: false`, unknown events, malformed payloads, or a missing channel service in a best-effort deployment return 200 with a skipped/handled response. Failure to initialize the receipt-backed channel path aborts a durable-profile startup. |
...
- `tests/test_durable_invocation_qualification.py` and `tests/test_durable_invocation_process_failures.py` are the cross-commit qualification boundary for replay, lifecycle agreement, failure recovery, worker/graph/first-stream counting, public filtering, and documentation. `tests/test_durable_invocation_contract_closure.py` pins the final concern-to-evidence matrix and the clarification, polling-observation, storage-profile, qualification, and deliberate-deferral language in `docs/INVOCATION_RUNTIME.md`. The `postgres_contract` marker covers the required independent-session admission, lifecycle-CAS/cursor, and repeatable-read tests and is a required CI gate whenever `DEERFLOW_TEST_POSTGRES_URL` is configured; a local skip is an unpassed release gate. Real-pod recovery is a separate opt-in `kubernetes_contract` suite in `tests/kubernetes/test_durable_invocation_pod_recovery.py`. Without exact `DEERFLOW_TEST_KUBERNETES=1`, it collects and skips with one explicit prerequisite message; when enabled, missing Helm/kubectl/KUBECONFIG/artifact inputs, any scenario skip, timeout, or incomplete evidence fails the session. The harness pins both CLIs to a doubly confirmed context and a newly created `hartmesh-qualification-*` namespace, keeps PostgreSQL and Redis outside the killed Gateway pod, and records accepted-before-response, accepted-before-worker, active, terminal-commit, graceful, and forced-after-deadline scenarios. Its runtime barriers and deterministic model are unreachable unless `DEERFLOW_TEST_KUBERNETES_RUNTIME=1` is injected by the test-only ConfigMap. Passing evidence is tied to image/chart/config/schema/environment identities and is published to the administrator deployment report only after every scenario passes. This qualifies one Gateway replica; it does not qualify scheduler HA, failover, active-active ownership, or zero-downtime rollout.
- `tests/test_invocation_lifecycle_characterization.py` is the regression boundary for the current durable invocation lifecycle, with source-specific routing pinned further by `tests/test_invocation_runtime_scheduled_tasks.py` and `tests/test_invocation_runtime_channels.py`. The normal graph-entry inventory is the stateless `/api/runs/stream` and `/api/runs/wait` routes; thread-scoped create, stream, and wait routes; native-channel wait, stream, fire-and-forget, and buffered-follow-up dispatch; and Scheduled Task dispatch. HTTP retains `services.start_run()` as its compatibility adapter; the in-process scheduler and native-channel manager construct `InternalLaunchIntent` directly. Every path converges on `InvocationRuntime.launch()` before worker attachment. Unkeyed launches continue through `RunManager.create_or_reject()`; keyed launches use `RunManager.ensure_or_reject()` -> `RunStore.ensure_run_atomic()`, whose SQL unique index is the race arbiter and whose only outcomes are `created`, `known_same`, and `key_conflict`. Only `created` attaches a worker. The scheduler decides when an occurrence should run, channels decide routing/delivery, and `InvocationRuntime` owns durable execution. The other production sources of durable `RunRow` records are temporary checkpoint-write and artifact-write reservations through `RunManager.reserve_thread_operation()`. `RunManager.create()` and missing-row persistence repair can use the idempotent `RunRepository.put()` constructor, but no inspected production entry point calls `RunManager.create()`; it remains a compatibility/testing primitive, and repair restores an already-admitted record rather than launching graph execution.
- `app/runtime/invocation.py::InvocationRuntime` owns the application-layer launch/observe/cancel lifecycle. Its finite host-internal operations normalize through explicitly injected collaborators, keep durable admission before single worker attachment, delegate visibility/cancellation, and expose authorized lifecycle queries without exposing `RunManager` to callers. Cross-owner service observation uses the injected `ObservationVisibilityResolver`: self-owner lookup stays the fast path, an authenticated service may receive only a current bounded `ServiceObservationGrant`, and `LifecycleVisibilityScope` is applied inside the store before a context page is materialized. The grant is search visibility, never binary permission; `AuthorizationProvider` remains the only `invocation:observe` allow/deny authority. The Module never accepts `FastAPI.Request`, reads `app.state`, or installs a process-global singleton; Gateway constructs adapters in `app/gateway/services.py`. `services.start_run()` remains the thin FastAPI compatibility adapter that maps the validated HTTP model to `InternalLaunchIntent` and preserves existing HTTP error translations. Keep the dependency direction `app -> deerflow`: neither the harness, `deerflow-extension-api`, nor `deerflow-runtime-api` may import this application Module.
- `deerflow-runtime-api==0.2.0` is the supported host-independent contract. Its `DurableInvocationPort` Protocol is the real seam shared by the in-process adapter and HTTP facade, with only exact `ensure`, invocation/context `observe`, fenced `control(cancel)`, and `capabilities` operations. Its public records recursively snapshot JSON mappings/sequences into immutable structures; `to_dict()` recursively returns a new mutable wire graph. `app/runtime/api.py` binds one host-authenticated principal and translates the Protocol to `InvocationRuntime`; callers cannot provide principal, scope, Origin, accepted digests, revision, config/context/metadata bags, delivery settings, or credentials. `InternalLaunchIntent` and `PreparedLaunch` likewise snapshot caller-owned configuration, checkpoint, context, callback collections, and other nested containers, and Gateway adapters thaw fresh copies only at normalization, persistence, and execution boundaries. The embedded adapter derives one service effective subject plus a canonical `service:v1:sha256:...` scope from `["service", authenticated_service_id]` and uses that same identity for start/observe/cancel; the HTTP adapter derives its identity and scope from Gateway authentication. `app/gateway/routers/runtime_api.py` is only a transport facade over that same adapter. Its five `/api/runtime/v1` routes enforce existing route permissions and owner/admin visibility, require an admin for capabilities, strictly decode package DTOs, cap pages at 1–500 (default 100), and translate every failure—including auth and CSRF middleware failures—into the fixed `runtime.error` envelope. Existing LangGraph-compatible routes remain independent compatibility facades.
- Identifier domains are field-specific and shared across configuration, persistence, trusted context, constraints, MCP, and portable DTOs. Custom-agent input matches `[A-Za-z0-9][A-Za-z0-9-]{0,127}` and is canonicalized to lowercase because agent storage has always been case-insensitive; `lead_agent` is the reserved built-in runtime identity, not a creatable custom agent. The legacy channel-session adapter still maps whitespace/case/underscores to the canonical lowercase hyphenated identity, but emits an actionable operator warning so that compatibility conversion is not silent. Thread IDs preserve exact case and match `[A-Za-z0-9_-]{1,64}`. Model-profile IDs preserve exact case/Unicode, reject ASCII controls, and are limited to 128 UTF-8 bytes. MCP server configuration keys use that same bounded non-control UTF-8 shape but are their own domain; MCP callable tool names preserve exact case and match `[A-Za-z0-9_-]{1,128}`. A non-tool-compatible server key must set `tool_name_prefix=false`; a prefix-enabled key is limited to 126 characters so `_` plus a non-empty tool name remain within the callable bound. Never reuse contributor/plugin identifier validation, truncate, hash, or silently alias one of these policy-visible identities. Overlong legacy model-profile names must be renamed in `models[].name` and every reference before startup; no implicit alias migration exists.
- Every HTTP, Scheduled Task, and authenticated native-channel launch is sealed before durable admission as one immutable host-owned `AcceptedInvocation`. Caller thread/assistant/agent/context values remain hints until Gateway resolution; native-channel source facts are built only after provider authentication and one verified source-binding path: interactive channels re-read the active connection and owner, while signed webhooks resolve an installation/owner/agent/repository route after HMAC verification. The accepted record owns a versioned `InvocationIdentityV1` with one effective human/service subject and an optional acting service, plus a separate bounded `InvocationOrigin`, resolved thread/context, normalized input/options, extension generation, and a `ResolvedAgentRevision` digest. Direct humans have no actor; channel and user-owned schedule launches keep a human subject with a channel/scheduler actor; system schedules and embedded calls use service subjects. Transport/source facts never imply subject authority. Gateway scrubs caller-supplied identity/actor/Origin flags, and start/observe/cancel authorization, contributors, constraints, MCP, and delegated subagents inherit the same sealed records. New rows persist principal projection v2 and both identity/source digests in the existing admission transaction. Hydration recomputes every derivable principal, base/final Origin, request/effective-execution, accepted-context, runtime-identity, and revision binding and reapplies fresh-seal cross-field invariants before a row can reach replay, authorization, recovery, or execution. Two present contradictory facts return only the stable `accepted_evidence_invalid` integrity result; they are never guessed equal. Historical rows whose evidence is wholly absent remain readable only through the explicit non-privileged compatibility path, and legacy `is_internal` is cleared for attributed channel users and non-service roles so compatibility can only remove privilege. Checkpoint/artifact reservation rows carry none of these facts.
- Normal `RunRow(operation_kind="run")` rows are also the durable invocation identity; there is no separate invocation table. Optional HTTP `Idempotency-Key`, trusted native-channel provider event/message ID, and Scheduled Task `task_run_id` map through `app/runtime/idempotency.py` to a bounded normalized key and a domain-tagged, canonical-JSON scope. HTTP scopes use authenticated user/service identity (or the configured auth-disabled default user); channel scopes use provider + verified binding kind/reference + workspace + chat; scheduler scopes use persisted owner + task. The `connection` binding retains the historical provider/connection/workspace/chat tuple byte-for-byte, while `webhook_route` uses a distinct kind-tagged tuple. Missing stable IDs or missing verified bindings stay unkeyed, and no source may synthesize a content hash. Keyed rows atomically persist two separate immutable projections: `caller-intent-canonical-json-v1` records normalized caller-supplied semantics used exclusively for replay equality, while `sha256-canonical-json-v1` records accepted effective execution including resolved thread/revision/principal/Origin/context/extension facts. Replay constructs fresh caller intent and never merges a sparse retry into stored effective values. The caller projection distinguishes removed non-null options, explicit versus server-assigned thread selection, graph versus resume input, and raw non-null recursion selection; it defines null as omission for nullable execution context/checkpoint/interrupt fields and default recursion, ignores mapping order, preserves sequence order, and excludes route/stream/delivery choices, callbacks, trace data, temporary credentials, and temporary attachment URLs. A visible equal retry returns the retained row in any status and reuses its accepted projection without rerunning defaults, aliases, contributors, start authorization, constraints, or work. Missing legacy caller evidence conflicts conservatively. A different key on an active thread remains `thread_busy` under `reject`, while `interrupt`/`rollback` retain their existing atomic supersession behavior. The guarantee lasts only while the row is retained. Tests: `tests/test_invocation_caller_intent.py`, `tests/test_invocation_idempotency.py`, `tests/test_verified_native_channel_bindings.py`, and `tests/test_migration_canonical_caller_intent.py`.
- Normal runs carry a non-negative `RunRow.state_version`; new admissions commit at version 1 with one `accepted` row in `run_lifecycle_events`. Every later authoritative normal-run mutation compare-and-sets the current version, increments it exactly once, and appends exactly one matching lifecycle row in the same transaction. The v1 lifecycle types are only `accepted`, `started`, `cancellation_requested`, `cancelled`, `succeeded`, `failed`, `timed_out`, and `interrupted`. `RunRow.status` remains authoritative—this journal is safe lifecycle evidence, not event sourcing. Historical and auxiliary operation rows start/remaining at version 0; checkpoint/artifact reservations never emit lifecycle evidence.
- Lifecycle cursors are global commit-order evidence. SQL writers lock the singleton `run_lifecycle_cursor_state(singleton_id=1)` row (`SELECT ... FOR UPDATE` on PostgreSQL; `BEGIN IMMEDIATE` before the read on SQLite), increment the cursor, mutate the run row, and insert the safe event in one transaction. Startup may repair a missing singleton only while the lifecycle table is empty; events without the singleton are ordering corruption and fail initialization. Event payloads contain only a version, an allowlisted host-owned reason code, and the allowlisted cancellation `action` evidence (`interrupt|rollback`)—never prompts, messages, reasoning, tool bodies, credentials, artifacts, or rich `RunEventStore` content. Lifecycle type/resulting-status pairs are validated before mutation. Authorized in-process and `/api/runtime/v1` queries return opaque versioned cursors, events in `(cursor, read_fence]`, `next_cursor`, `minimum_available_cursor`, the captured global fence, and `InvocationSummaryV1` rows joined from accepted normal runs. A summary is the single public source/evidence representation: current state, sealed source kind, bounded safe Origin correlation references, agent/extension identity, and accepted evidence digests; it never duplicates into each event. The portable observation enforces page relationships on both direct construction and parsing: rows match the observed thread/run, snapshot and summary run IDs are unique, summaries join to same-page snapshots with identical current state, and singular snapshots match the top-level state; historical event states remain transition evidence. Context queries may filter only the strict sealed source kinds, and owner/observe authorization applies first. PostgreSQL reads start a read-only `REPEATABLE READ` transaction before their first SELECT; SQLite uses one explicit read transaction, so events and summaries share a snapshot. Context queries load summary/snapshot rows only for distinct run IDs in the bounded event page (1–500), not every run in the thread; lifecycle payloads are capped at 4 KiB, summaries at 16 KiB, and portable observations at 12 MiB. Historical rows without sealed Origin remain event/snapshot-readable without a summary. Empty filtered pages advance to the fence, page-boundary cursors stop at the last returned event, and gap/ahead errors are typed. Administrative pruning is explicit and monotonic; there is no timer, retention worker, or broker.
...
- Atomic admission results are a cancellation boundary. A durable store can commit before its coroutine returns the materialized row, so `RunManager` executes `ensure_run_atomic()` and `create_thread_operation_atomic()` in shielded tasks and drains their definite outcome while holding its admission lock. If the caller was cancelled after a newly created row committed, the manager first registers/synchronizes the committed rows and then terminalizes the unseen replacement, or deletes an unseen auxiliary reservation, before re-raising cancellation. The cancellation flag is retained even when the decision raises, so a retryable uniqueness race cannot launch another admission after cancellation. A cancelled `known_same`/`key_conflict` lookup never closes the retained run. Keep this logic centralized in `_await_atomic_admission_result()` and `_drain_cancelled_admission_cleanup()`; directly awaiting a new atomic admission primitive can reintroduce a durable pending row with no worker.
...
- `cancel()` returns a :class:`~deerflow.runtime.CancelOutcome` enum: `cancelled` (local cancel), `requested` (the non-owning worker durably recorded the first cancellation action for the live owner), `taken_over` (non-owning worker claimed the run because the owner's lease expired—marks it as `error` with `orphan_recovered`), `lease_valid_elsewhere` (legacy/custom store lacks the durable request primitive—caller retains the safe 409 + `Retry-After` fallback), `not_active_locally` (heartbeat disabled, preserving the old 409 path), `not_cancellable` (terminal state), or `unknown` (not found in memory or store).
- Store-only hydrated runs are readable history. `request_cancel_fenced()` accepts an expected version and returns only `requested`, `already_requested`, `already_terminal`, `stale`, or `not_found_or_invisible`; after visibility, same-action replay wins precedence over stale version, terminal wins over version mismatch, and a different recorded action or mismatched version is stale. The existing route uses the separate atomic unfenced `request_cancel_compat()` operation, so pending-to-running races cannot manufacture a read/CAS conflict. Only the first request increments `state_version` and emits `cancellation_requested`; completion and cancellation finalization are competing CAS writes, so exactly one terminal outcome wins. `RunStore.renew_lease()` renews and observes the request atomically in the SQL implementation. The owner executes normal process-local interrupt/rollback and emits `cancelled` without transferring the lease. `wait=true` and cancel-then-stream use the shared bridge to observe owner finalization; a non-standard process-local bridge returns accepted 202 instead of subscribing to an unreachable stream. In single-worker mode (heartbeat off), store-only runs still return 409.
...
- Startup/orphan reconciliation must claim stale active rows with `RunStore.claim_for_takeover()`, not a plain `update_status()`. Durable lifecycle stores also fence the claim with the scanned `state_version`; the final claim re-checks version, status, and lease expiry atomically, so a heartbeat renewal, cancellation, or transition between the candidate scan and the recovery write keeps the stale claim from writing.
...
**Provider Pattern**: `SandboxProvider` with `acquire`, `acquire_async`, `get`, `release`, and the accepted-skill acquisition/binding lifecycle. `AcceptedSkillSandboxBindingV1` distinguishes a selected digest from an explicitly empty accepted set; absence of accepted material is the legacy path. Durable execution uses the bound accepted acquisition seam so a remote provisioner receives the committed evidence before it creates a Pod; eager middleware and lazy/deferred tools use the same seam. Mutable live skill trees must be absent at sandbox creation, and cached reuse revalidates the exact binding. Accepted-only namespace isolation is separate from immutable material support. The conservative base capability is `empty_only`; a provider may advertise `immutable_read_only` only when commands in that sandbox cannot mutate the accepted bytes. Async agent/tool paths call async sandbox lifecycle hooks so Docker sandbox creation, discovery, cross-process locking, readiness polling, binding, and release stay off the event loop.
...
**Implementations**:
- `LocalSandboxProvider` - Local filesystem execution. `acquire(thread_id)` returns a per-thread `LocalSandbox` (id `local:{thread_id}`) whose `path_mappings` resolve `/mnt/user-data/{workspace,uploads,outputs}` and `/mnt/acp-workspace` to that thread's host directories, so the public `Sandbox` API honours the `/mnt/user-data` contract uniformly with AIO. `acquire()` / `acquire(None)` keeps the legacy generic singleton (id `local`) for callers without a thread context. Per-thread sandboxes are held in an LRU cache (default 256 entries) guarded by a `threading.Lock`; invocation-owned entries are not evictable, so the cap is temporarily soft when every candidate still has a lead or background consumer. Public, custom, legacy, and managed integration skill mappings point at stable enabled-only projection roots rather than raw skill directories. Durable accepted-only Local instances exclude mutable live skill trees, but Local subprocesses share the Gateway identity and therefore cannot prove nonempty accepted bytes immutable. Local advertises `empty_only`; nonempty durable accepted skills fail before model work. Use Docker/AIO for that production path. Legacy Local instances retain their existing command behavior.
- `AioSandboxProvider` (`packages/harness/deerflow/community/`) - Docker/Kubernetes isolation. Local Docker supports nonempty accepted snapshots by mounting the selected active view read-only. Remote/provisioner AIO defaults to `empty_only`; it advertises `immutable_read_only` for a sandbox only after an `rwx_verified_copy_v2` request yields a complete materialization receipt. V1 receipts remain parseable compatibility records but are always `empty_only`. The provisioner mounts one content-addressed snapshot subpath from a cross-node RWX home claim into a verifier init container only, performs bounded symlink-safe copy and canonical digest checks, publishes into a private per-Pod `emptyDir`, and mounts only that copy read-only into the sandbox. Mutable live skill volumes and source aliases are absent. V2 binds the Pod UID and canonical admitted isolation projection (containers/init containers, images, commands, mounts, volumes, ports, security contexts, host namespaces, ServiceAccount/automount, runtime class, DNS, restart policy, and ephemeral containers), Lease UID/attempt, exact NetworkPolicy UID/spec digest, immutable evidence/capability Secret UIDs/digests, pinned runtime images, accepted run/snapshot/generation, verifier receipt, and final materialization digest. A per-attempt capability protects direct Pod-IP access; it is held only in Gateway process memory and the Lease-owned immutable Kubernetes Secret for that Pod attempt, and never enters RunRow, lifecycle events, checkpoints, or logs. One native Kubernetes Lease is the attempt owner root: the Pod, immutable evidence/capability Secrets, and NetworkPolicy share its UID. Response-loss replay, cache/warm reuse, Lease renewal, and the worker pre-stream fence re-read the complete tuple and fail closed on deletion, replacement, malformed evidence, or drift; renewal happens only after authoritative RunRow renewal. A bounded reconciler deletes expired owner roots. Gateway management requests use a rotating audience-bound projected ServiceAccount token, validated by TokenReview against the exact Gateway namespace and ServiceAccount; authenticated profile preflight participates in readiness. The Helm profile requires digest-pinned sandbox and verifier/gate images and rejects RWO rather than adding a same-node requirement; provisioner readiness checks the actual claim is `Bound` and reports `ReadWriteMany`. Accepted Pods only prefer a different Gateway node when one is available. Fake-Kubernetes and Helm-render tests prove construction and drift rejection, not live cross-node CNI/RWX execution; exact-artifact `kubernetes_contract` evidence remains an external release gate. An accepted Pod cannot be adopted after process loss because its capability is intentionally unavailable, matching orphan terminalization. `get()` remains an in-memory lookup for event-loop-safe tool paths. `uses_thread_data_mounts` defaults to backend detection (`LocalContainerBackend=True`, remote/provisioner backends=False), while `sandbox.thread_data_mounts` may assert shared thread data. Readiness and sandbox clients disable ambient HTTP proxies for control-plane destinations.
- `E2BSandboxProvider` (`packages/harness/deerflow/community/e2b_sandbox/`) provides E2B remote isolation.
  ...
  legacy, and managed integration projections. Existing E2B VMs keep the live
  enabled-only projection from creation because E2B has no shared host mount,
  The accepted-profile machinery is isolated from legacy discovery and
  quarantines an unclearable VM, but E2B does not currently prove the uploaded
  tree immutable against sandbox commands. Durable execution is therefore
  empty-only and fails before model work when effective skills are present.
...
- **Tool policy**: Agent `allowed-tools` declarations apply dynamically only to slash-activated skills and skills captured in `ThreadState.skill_context` through configured `read_file` loads; passive enabled skills and custom-agent/subagent skill allowlists remain discoverable without clamping the baseline toolset. Subagents render only skill discovery metadata at startup and reuse the same adjacent `SkillActivationMiddleware` + `SkillToolPolicyMiddleware` pair as the lead; their configured `skills` field limits discovery and activation instead of eagerly loading bodies or unioning policies. Slash policy is dominant for its run, preventing subsequently read skills from widening explicit authority; autonomous captured skills use the existing union only when no slash source exists. `tool_search` and `describe_skill` stay available as framework discovery infrastructure, while every discovered or promoted business tool still requires active-policy permission for schema visibility and execution; `task` likewise requires an explicit declaration. Durable runs resolve policy from their accepted immutable skill snapshot, including an explicitly empty skill set, so a later enable/disable or frontmatter edit cannot widen, revoke, or replace accepted material. Legacy embedded runs without accepted material retain live-registry reload behavior. All tool calls produced by one model step reuse the resulting source-and-path-signed decision. Registry/snapshot failures and all-invalid active sets fail closed, while stale individual paths are skipped when another valid skill remains. This is best-effort behavioral scoping, not a hard security boundary: alternate loading paths are not captured and bounded autonomous context may evict entries.
- **Accepted skill snapshot**: `runtime/skill_snapshot.py` is the sole owner of accepted skill-tree copy, hashing, atomic publication, verification, reference-counted leases, and cleanup. `resolve_agent_revision()` snapshots all non-symlink regular files in every effective skill package off the event loop and derives the revision's skill projection from those exact bytes and executable flags. It rejects unreadable/changing trees, symlinks, special files, path escape/duplication, and the documented per-file/per-skill/aggregate bounds. Snapshot-backed `Skill` records feed lead prompts, slash activation, deferred discovery, policy/secret metadata, subagents, and sandbox access. Bodies and host paths are process-local and never enter `RunRow`; only bounded metadata/digests do. A background subagent takes its own lease before dispatch, so parent terminalization cannot remove material it still uses. Terminal workers, task-completion callbacks, and failed attachment release leases; equal snapshots share safely, and Gateway startup removes abandoned process-local trees. Process-loss recovery terminalizes the worker rather than reconstructing historical bytes.
- **Sandbox projection**: `skills/projection.py` materializes enabled-only live trees at `{base_dir}/skills_view/public` and `{base_dir}/users/{user_id}/skills_view/{custom,legacy,integrations}` for legacy execution. A durable accepted run instead uses an accepted-only acquisition profile with no mutable live skill tree. Local Docker AIO advertises `immutable_read_only` through its read-only accepted view. Remote AIO does so only for a complete receipt-backed `rwx_verified_copy_v2` private verified copy; v1/incomplete receipts remain `empty_only`. Writable configured host mounts that alias accepted material are rejected. Local, E2B, and custom providers remain `empty_only`. File/search validation enforces the exact accepted subtree as defense in depth, not as a substitute for provider immutability. `runtime/skill_projection.py` reserves each user/thread before acceptance, promotes only the created run, and issues exact run/generation/consumer tokens. Lead and background subagent consumers share one invocation fence. Last-consumer release enters a clearing phase that remains busy until the provider proves the old bytes cleared, never published, or quarantined and CAS-finalizes ownership; failed clear attempts may retry with the same exact token, and a later run cannot bind while cleanup is in flight. An unproven custom-provider failure remains busy and cannot release or recycle its sandbox. An explicit empty binding contains no digest. Live enabled-only projections still hardlink files when possible and fall back to copies across filesystems. Storage writes, archive installs, deletes, and toggles rebuild under a cross-process lock; Gateway boot ensures only the shared public view, while each user view is repaired lazily on first legacy sandbox acquire. Managed integration packages are global, but their projected category is per-user because enabled state is isolated. Rebuilds stage a complete tree and reconcile it with per-file atomic replacement, so unrelated enabled skills remain continuously visible; disable/delete paths remove only the affected package before mutating to preserve fail-closed behavior. User projection rebuilds re-read global enable state from disk instead of the process singleton, so a toggle handled by one worker is honored by another worker's next sandbox acquire. Gateway public-skill toggles take the public projection lock before the shared `extensions_config_write_lock`, re-read an existing config from disk, persist the full model shape, and rebuild before responding; keep this as one worker-owned critical section so MCP writes cannot interleave and request cancellation cannot release either lock while the worker still runs. The shared public steady-state signature check runs without the global projection lock; stale/error paths take the lock and re-check before rebuilding or clearing. User-scope checks remain serialized per user. Category root inodes remain stable so live bind mounts observe content changes without sandbox recreation. Projection failures clear the affected view before raising.
...
- **Slash activation**: `/skill-name task` loads that enabled skill's accepted `SKILL.md` for the current model call only on durable runs; legacy embedded calls use the live registry. The resolver rejects leading whitespace, missing separators, reserved channel commands (`/new`, `/help`, `/bootstrap`, `/status`, `/models`, `/memory`, `/goal`), disabled skills, and skills outside a custom agent's whitelist.
...
**Architecture**: In the embedded Gateway process, channels receive an explicitly constructed `InvocationRuntime` for durable run launch. They retain the `langgraph-sdk` HTTP adapter for thread create/update and for joining admitted results/streams, so those compatibility operations remain server-owned; a standalone manager with no injected runtime also retains SDK run launch for a real process boundary. The internal SDK client injects process-local internal auth plus a matching CSRF cookie/header pair so Gateway accepts state-changing thread requests and run observation without relying on browser session cookies.
...
1. External platform -> Channel impl -> `MessageBus.publish_inbound()`
   - For signed GitHub, the webhook router verifies HMAC and registry bindings, then commits one bounded `inbound_receipts` row per matching binding in a single transaction before returning success. It separately binds the exact HMAC-verified body plus bounded host-accepted event header to `provider_event_digest`; for the same currently resolved binding-and-delivery receipt key, an equal redelivery reuses the first stored launch envelope even if mutable agent policy changed, while different evidence returns a bounded permanent 409. A no-target/no-match event produces no candidate and therefore has no delivery-wide receipt identity; fan-out membership is not globally frozen. Only receipt IDs are published to `MessageBus`; the bus is a wake-up path, not the acknowledged copy. A storage failure returns a retryable response. `native_ingress.v1` reports durable only when startup-frozen verified-route eligibility, current HMAC mode, and PostgreSQL receipt storage are all present; readiness uses that same intersection. Adding a previously absent secret requires restart, while rotating a configured nonblank secret remains request-time behavior. Explicit unverified development webhooks remain best-effort with every database backend and are unavailable under `durable_production`.
...
4. For chat: look up/create the thread through Gateway's LangGraph-compatible transport, then build the typed native-channel launch intent only after provider authentication, connection lookup, active-owner revalidation, and agent/thread selection. `ChannelManager` is explicitly constructed with the process-local `InvocationRuntime`; every durable wait, stream, fire-and-forget, and buffered-follow-up launch enters that shared boundary. A directly constructed standalone manager without the runtime retains the SDK launch transport for a real process boundary.
5. Feishu/Telegram chat: launch through `InvocationRuntime`, join the admitted run's stream, accumulate AI text, publish multiple outbound updates (`is_final=False`), then publish the final outbound (`is_final=True`).
6. Slack/Discord chat: launch through `InvocationRuntime`, join the admitted run to its final state, extract the response, then publish outbound.
6b. GitHub chat (`ChannelRunPolicy.fire_and_forget=True`): a receipt claimant reconstructs the immutable message, launches it through `InvocationRuntime`, and binds the created or known-equal run ID with the current fencing token. A busy thread defers the same receipt in durable FIFO order; it does not create a second process-local follow-up copy. The agent posts its own reply mid-run via `gh` from the sandbox. Best-effort/local dispatch retains the existing process-local follow-up buffer.
...
- **Native-channel trust sequence**: the provider adapter authenticates the platform event before publishing an `InboundMessage`. For interactive IM, the host re-reads the connection by provider identity and revalidates both the active `connection_id` and DeerFlow `owner_user_id`; the ordinary path does that in `ChannelManager`, while Buzz's signed relay adapter does it through the shared `attach_connection_identity()` helper before its channel-specific manager policy. Only that repository result may mint `InternalVerifiedNativeBinding(kind="connection")`; the ordinary manager path deliberately replaces any claimed binding with the re-read row. For signed webhooks, the route creates a request attestation only after HMAC verification and the dispatcher combines it with a trusted registry match for strict-positive installation, owner, canonical agent, and repository to mint `kind="webhook_route"`; it leaves `connection_id=None`, so thread persistence never touches the interactive connection repository. Durable keyed receipt admission additionally requires a provider delivery ID plus the digest of the HMAC-verified body and host-accepted event header; neither comes from message metadata, and a fired signed route missing either fails before acknowledgment. Keyed admission requires exactly one coherent binding. The safe kind/reference enter the sealed Origin and shared trusted run context, while raw payload metadata cannot mint or replace them. The raw platform sender remains `channel_user_id`, separate from the authenticated DeerFlow owner; a human webhook subject is not relabelled as a service. A top-level manager-owned `agent_name` may select a configured route, while similarly named fields inside the raw provider payload cannot select an agent.
- **Stable provider ID rule**: native-channel facts carry the first non-empty server-stable `event_id`, `message_id`, or `msg_id` from manager-owned metadata (then the same keys from the raw provider envelope). When a provider supplies none, `provider_message_id` is `None`; never synthesize a content hash or client-generated fallback. The manager/Postgres TTL dedupe store uses the same stable-ID extraction as a bounded fast-path optimization. The trusted ID becomes the runtime external key only when the message also has a verified source binding, so a redelivery that bypasses or outlives TTL dedupe converges at `RunStore.ensure_run_atomic()` without giving unverified development traffic a production scope. This durable guarantee lasts with the retained `RunRow`.
...
- Custom agents declare a `github:` block in their `config.yaml` to bind to repos and event triggers; the webhook route is fail-closed by default (mounted only when `GITHUB_WEBHOOK_SECRET` is set) and exempt from auth/CSRF because authenticity is enforced by HMAC. A signed production delivery fires a bound agent only when its payload installation matches the agent's configured positive `installation_id`; the resulting route binding is derived from provider/installation/owner/agent/repository and is distinct from an interactive connection. Explicit unverified development mode remains available but cannot create a keyed verified-binding scope.
...
- **Leased inbound receipts and busy follow-ups**: in a PostgreSQL-backed signed GitHub deployment, `InboundReceiptProcessor` owns the pre-admission state machine: `received -> claimed -> admitted(run_id) -> completed`, with `deferred` and bounded `dead_letter` outcomes. Claims have an expiry and monotonically increasing fencing token; only the current claimant may bind, defer, or complete. `attempt_count` records claims, while `failure_count` records only malformed/processing failures. `thread_busy` is ordinary contention: it returns the exact row to `deferred` on a fixed bounded cadence without incrementing `failure_count`, so it can outlive the poison horizon and process restarts. Poison failures use bounded exponential delay and terminalize at the configured failure limit; completed/dead-letter rows have explicit retention cleanup. Recovery scans bounded pages, reclaims expired claims, and uses the stable verified-binding/provider-delivery identity to resolve a run accepted before a crash. Only the earliest unfinished receipt for a thread is claimable, so a busy receipt remains the durable FIFO source of truth until later admission. Commands and permanent rejections complete without inventing a run. The older `ChannelManager._followup_buffers` FIFO remains only for explicitly best-effort/local dispatch and has the documented process-loss boundary.
- **Owner-scoped GitHub conversations**: signed deliveries derive both `preferred_thread_id` and `topic_id` from one v2 projection of the verified route binding, repository, issue/PR number, and canonical agent. Same-named agents belonging to different owners cannot share a mapping, checkpoint, receipt FIFO, or thread. Unverified development retains the legacy v1 mapping. A legacy signed thread is never transferred or implicitly aliased; only a row whose persisted owner is SQL `NULL` may be atomically claimed, and any non-null different owner fails before graph/model execution.
- **Owned-thread execution fence**: an admitted worker must establish or re-read thread metadata for its exact effective owner before graph/model execution. Ownership conflict, metadata-store failure, or the bounded setup timeout terminalizes the still-pending run with stable `worker_attachment_failed` evidence, closes attached and late stream consumers, and emits only bounded correlated diagnostics; raw store exceptions never enter logs. Only an explicitly ownerless legacy record may keep the historical fail-open metadata behavior. A cancellation committed before this fence terminalizes wins, closes the stream, and never enters graph preflight; attachment failure cannot overwrite it.
- See [backend/docs/GITHUB_AGENTS.md](docs/GITHUB_AGENTS.md) for the architecture diagrams: webhook → fan-out → `InboundMessage` dispatch, owner-scoped v2 conversation identity, mention-handle precedence chain, GH token lifecycle via `GH_TOKEN`/`GITHUB_TOKEN` per-call `extra_env`, and the narrow `ConflictError` (HTTP 409) thread-create race recovery.
...
- `shutdown_flush_timeout_seconds` - Hard budget (seconds) reserved for draining the memory backend's pending-update buffer on Gateway graceful shutdown (default: 30; 1–300). `app.gateway.shutdown.GracefulShutdownCoordinator` is the one production owner: it freezes admission, stops channel/scheduler producers, drains locally active runs, then flushes memory before closing retrieval, browser, OIDC, stream, and database resources. `deployment.shutdown` owns the other phase budgets; their sum plus this field is one absolute application deadline. Repeated calls share one result. If admission or run quiescence is unproven, memory is not flushed or closed while a late writer may exist. The Helm chart derives its termination grace from the application deadline plus preStop and scheduling headroom. This is single-replica graceful behavior and existing process-loss/orphan recovery, not live-pod qualification or multi-replica coordination.
...
- `migrations/versions/0011_mcp_tasks.py` — upstream predecessor that adds persisted MCP task state; durable-invocation rollback must stop here so unrelated MCP-task rows survive
- `migrations/versions/0011_accepted_invocation.py` — chains after `0011_mcp_tasks` and adds nullable host-sealed acceptance facts and pinned agent revision evidence for normal runs
- `migrations/versions/0012_invocation_idempotency.py` — adds nullable external identity/request-digest columns, consistency checks, and `uq_runs_external_identity`; historical and auxiliary rows remain unkeyed
- `migrations/versions/0013_invocation_lifecycle.py` — adds authoritative state versions, lifecycle evidence, and the locked global lifecycle cursor
- `migrations/versions/0014_canonical_caller_intent.py` — adds nullable canonical caller-intent projection/digest columns; legacy rows remain readable but cannot prove keyed replay equality
- `migrations/versions/0015_inbound_receipts.py` — adds bounded leased native-ingress receipts, source-delivery uniqueness, fencing, deferred retry, run binding, and terminal outcome evidence; the existing `0009` TTL dedupe table remains backward-readable best-effort compatibility
- `migrations/versions/0016_sandbox_execution_evidence.py` — adds the nullable bounded execution-evidence JSON/digest pair used to atomically bind an accepted Kubernetes skill materialization to the `started` lifecycle transition; auxiliary rows cannot carry it
- `migrations/versions/0017_lifecycle_integrity.py` — adds transactionally maintained retained lifecycle cardinality used by bounded readiness integrity checks
- `migrations/versions/0018_inbound_receipt_failures.py` — separates poison `failure_count` from total receipt claims so ordinary thread contention cannot exhaust a valid delivery
- `migrations/versions/0019_inbound_event_identity.py` — adds nullable exact verified-request evidence; new signed receipts require it, while legacy rows remain processable without invented replay proof
...
`0011_mcp_tasks`, including a representative MCP task, through every invocation revision to
`0019_inbound_event_identity`, then exercises repository replay, cancellation, lifecycle
ordering, orphan takeover, summary reads, and receipt semantics. The supported feature-tail
downgrade stops at `0011_mcp_tasks`; the MCP task survives upgrade, downgrade, and re-upgrade.
Invocation-only columns, lifecycle rows, receipt rows, and their evidence cannot be represented
at that predecessor and are deliberately lost. A further technical downgrade to
`0010_run_cancel_request` invokes `0011_mcp_tasks.downgrade()` and is separately destructive to
MCP-task state; it is not the supported invocation rollback. Quiesce writers and take a database
backup before any rollback. The CI
...
`DeerFlowClient` provides direct, synchronous, non-durable local graph execution without HTTP services. It does not enter `InvocationRuntime` and does not claim Gateway-equivalent durable admission, recovery, cancellation, or lifecycle observation. The separate application-hosted in-process runtime Adapter implements the portable `DurableInvocationPort` over `InvocationRuntime`; helper response-schema alignment does not make these two durability surfaces identical.
