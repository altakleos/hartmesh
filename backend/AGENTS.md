...
- **Stable provider ID rule**: native-channel facts carry the first non-empty server-stable `event_id`, `message_id`, or `msg_id` from manager-owned metadata (then the same keys from the raw provider envelope). When a provider supplies none, `provider_message_id` is `None`; never synthesize a content hash or client-generated fallback. The manager/Postgres TTL dedupe store uses the same stable-ID extraction as a bounded redelivery optimization only. It is not durable invocation idempotency and deliberately creates no external key or accepted-origin record.
