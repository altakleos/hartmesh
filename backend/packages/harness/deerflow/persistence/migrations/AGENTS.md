...
- `migrations/versions/0027_multi_gateway_topology.py` — current head; adds the tenant/profile/replica topology registry and the persisted scheduler generation used by deterministic occurrence identity. In `durable_two_gateway_v1`, a Helm pre-install/pre-upgrade Job uses the exact Gateway image and PostgreSQL advisory lock to migrate. Gateway pods call the read-only head verifier and must never fall back to concurrent startup migration.
