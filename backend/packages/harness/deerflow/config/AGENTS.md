...
- `tool_plane` - Startup-frozen governed revision switch and policy: allowed MCP transports, stdio launchers, endpoint hosts/private-address policy, candidate count limits, and required skill review. Its canonical digest is recorded in validation evidence; edits require restart and revalidation.
...
- `mcpServers` - Map of server name → config (enabled, type, command, args, env, url, headers, oauth, description, `routing`, `tools`, `tool_call_timeout`, `session_init_timeout`, `credential_binding_id`, `credential_version`). `routing.mode="prefer"` emits `<mcp_routing_hints>` prompt guidance; if `tool_search` defers the hinted tool, `McpRoutingMiddleware` can also auto-promote matching deferred schemas before the model call. It does not hard-disable other tools. `session_init_timeout` (default `DEFAULT_MCP_SESSION_INIT_TIMEOUT` = 60s, `null` to disable) bounds server bring-up: tool discovery and persistent stdio session initialization, so a hung server cannot block agent construction indefinitely; durable HTTP/SSE task calls use it for their ephemeral session initialization too. `tool_call_timeout` bounds individual stdio calls and durable-task calls on every transport; other HTTP/SSE tools use transport-level timeouts. For task-enabled servers, `credential_binding_id` is a non-secret operator identity and `credential_version` defaults to 1; durable lineage stores only their tenant/principal/server-scoped commitment and recovery fails safely when the current binding/version no longer matches.
...
When `tool_plane.enabled=false`, legacy Gateway endpoints and `DeerFlowClient` methods may modify MCP servers and skill state at runtime through the shared atomic replacement helper. With governance enabled (the default), those direct mutations fail and only validated promotion projects `extensions_config.json`; `middlewares` remains a separate operator-controlled config-file extension point.
