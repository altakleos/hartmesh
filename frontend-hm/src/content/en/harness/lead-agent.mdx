...
Plan mode is appropriate for complex, multi-step tasks where showing incremental progress is valuable. For simple requests, it is better left disabled to avoid unnecessary overhead: a model given the `write_todos` tool tends to spend turns on it even for a one-step task, so the workspace sends `is_plan_mode: true` only in Ultra mode.
