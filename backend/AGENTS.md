...
Infrastructure fields are **restart-required**. The authoritative list lives in `packages/harness/deerflow/config/reload_boundary.py::STARTUP_ONLY_FIELDS` and is mirrored by the standardised `"startup-only:"` prefix on the corresponding `Field(description=...)` in `AppConfig`, so IDE hover on those fields surfaces the reason inline (no need to context-switch into this table). Currently registered: `plugins`, `database`, `deployment`, `checkpointer`, `run_events`, `stream_bridge`, `sandbox`, `log_level`, `logging`, `channels`, `channel_connections`, `scheduler`, `mcp_tasks`, `run_ownership`. Adding a new restart-required field requires updating the registry; drift is pinned by `tests/test_reload_boundary.py`.
...
1. External platform -> Channel impl -> `MessageBus.publish_inbound()`
   - For signed GitHub, the webhook router verifies HMAC and registry bindings, then commits one bounded `inbound_receipts` row per matching binding in a single transaction before returning success. Only receipt IDs are published to `MessageBus`; the bus is a wake-up path, not the acknowledged copy. A storage failure returns a retryable response. `native_ingress.v1` reports durable only when startup-frozen verified-route eligibility, current HMAC mode, and PostgreSQL receipt storage are all present; readiness uses that same intersection. Adding a previously absent secret requires restart, while rotating a configured nonblank secret remains request-time behavior. Explicit unverified development webhooks remain best-effort with every database backend and are unavailable under `durable_production`.
...
`0018_inbound_receipt_failures`, then exercises repository replay, cancellation, lifecycle
