...
6b. GitHub chat (`ChannelRunPolicy.fire_and_forget=True`): `InvocationRuntime.launch()` returns once the run is durably admitted and its worker is attached; the manager does not wait for the final state and does not publish an outbound. The agent posts its own reply mid-run via `gh` from the sandbox. `ConflictError` on a busy thread still trips the standard `THREAD_BUSY_MESSAGE` path (log-only on GitHub); when the channel's policy also sets `buffer_followups_on_busy=True` (GitHub's default — see "Follow-up buffering while busy" below), the triggering message is additionally captured into a per-thread buffer instead of only logged, so a concurrent comment is not silently dropped.
...
- **Follow-up buffering while busy** (issue #4121): because outbound is log-only, the pre-existing `THREAD_BUSY_MESSAGE` reply on a `ConflictError` is invisible to a GitHub commenter. With `ChannelRunPolicy.buffer_followups_on_busy=True`, the manager retains one bounded, immutable source snapshot per delivery in a per-thread FIFO (`ChannelManager._followup_buffers`). The snapshot includes the delivery's stable provider ID, verified binding, owner, sender, route/agent selection, and already-built launch projections; it excludes raw provider payloads, attachment bytes, and transient credentials. The FIFO remains capped at 20 (oldest dropped with a WARNING on overflow). Each `END_SENTINEL` drains exactly one delivery through the same `InvocationRuntime`, on the same DeerFlow thread but under that delivery's own external key and sealed Origin. The chained run is watched before the next delivery drains. A same-key/equal replay returns the retained invocation without another worker; a same-key/different-intent result is a one-shot integrity diagnostic, not an infinite retry. If another run wins the thread, the exact entry is requeued at the head without mutation; when the in-process conflict identifies the active run, the manager watches it and retries after completion. **Plumbing**: `app.py` constructs the channel runtime and passes it through `start_channel_service(...)` → `ChannelService` → `ChannelManager`; the StreamBridge accessor follows the same path. A manager without a stream accessor preserves the buffer but cannot auto-drain it. **Scope limitation**: buffer and watcher state remain per-process memory. Process loss can therefore lose entries that were buffered but not yet admitted; the supported one-Gateway-replica topology avoids cross-worker routing but does not make this pre-admission queue durable.
