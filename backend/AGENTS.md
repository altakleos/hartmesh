...
- `packages/harness/deerflow/retrieval/` owns the provider-neutral external-retrieval policy, protected provider port, safe source normalization, host-only draft handoff, observation contract, concurrency bound, and low-cardinality metrics. RAGFlow, DuckDuckGo, Serply, and Tencent WSA opt in through explicit tool metadata. The outer `ToolReceiptMiddleware` remains the sole owner of the exact post-sanitization/post-budget result digest and uses `RunEventToolReceiptSink.append_retrieval_pair()` to commit the terminal receipt plus `retrieval.observation.v1` atomically. Never add a query/query-derived value or provider result text to receipts, events, logs, API projections, or metric labels; never infer MCP retrieval from names or payloads. The authorized run projection is `GET /api/threads/{thread_id}/runs/{run_id}/retrieval-observations`. See `docs/EVIDENCE_BEARING_RETRIEVAL.md` and the scoped runtime/middleware guides.
