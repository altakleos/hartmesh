...
- `assemble_lead_agent(config, *, app_config=None) -> LeadAgentAssembly(graph, descriptor)`
  ...
  Ordinary consumers may unwrap `.graph` defensively with `unwrap_agent_graph()`;
  the durable worker uses `split_agent_factory_result()` so it can retain the
  descriptor. Third-party factories may still return a bare graph for ordinary
  direct/non-durable use, but accepted durable execution rejects that shape before
  checkpoint or graph invocation.
- Descriptor construction remains conditional. It runs when an assembly observer
  exists or when trusted worker context contains the exact opaque sentinel installed
  under `REQUIRE_ASSEMBLY_EVIDENCE_CONTEXT_KEY`; a request-provided boolean or value
  with the same spelling is never sufficient. Observer notification still happens
  only when observers exist. Keep every descriptor, tool, and middleware field
  explicitly classified by `runtime/assembly_evidence.py`. Actual skill content
  uses `assembly_descriptor.skill_catalog_digest()` over exact manifest bytes;
  accepted expectations use `skill_catalog_digest_from_snapshot()` over frozen
  snapshot projections. `subagent_release_policy()` is the shared canonical
  policy projector for both graph assembly and accepted evidence.
...
- With `RESOLVED_AGENT_MATERIAL_CONTEXT_KEY`, subagent discovery and descriptions,
  allowed names, frozen limits, and the assembly policy's catalog digest come only
  from `ResolvedSubagentCatalogV1`. Do not read the registry on that path or accept
  a catalog-shaped client context value. Ordinary/non-durable factories retain live
  registry resolution.
- Accepted subagent assembly receives the selected typed definition and parent
  catalog digest. Its descriptor records both under effective policy, while skill
  loading uses only `subagent:<name>` content digests from the accepted scope map.
