...
# ── Login throttling ───────────────────────────────────────────────────────
# The lockout itself is keyed on the account and lives in
# ``app.gateway.auth.login_throttle``; its behaviour, the per-source spray
# guard, the admin unlock, the enumeration and timing properties, and the
# store's failure mode are all in tests/test_login_throttle.py. What stays
# here is how the *policy* is read, because that is a property of this router.


@pytest.mark.asyncio
async def test_login_throttle_policy_uses_defaults_when_config_unavailable(monkeypatch):
    ...
    because config.yaml is absent in bare-app contexts that never load one.
    """
    ...
    policy = auth_router._login_throttle_policy()
    assert (policy.account_max_attempts, policy.account_lockout_seconds) == (5, 300.0)


def test_login_throttle_malformed_config_propagates(monkeypatch):
    ...
    an operator who tightened the policy must never silently get the default.
    """
    from app.gateway.routers import auth as auth_router
    from deerflow.config import app_config as app_config_module
...
def test_login_local_broken_config_fails_closed_before_authenticate(monkeypatch):
    """Route-level pin: a malformed config never reaches password verification.

    Policy resolution happens before any store lookup, so a broken config.yaml
    500s on the way in. No password is verified while the file stays broken —
    previously the first request per address was verified before the counter
    could refuse it.
    """
    ...
    login_throttle.set_login_throttle_store(login_throttle.MemoryLoginThrottleStore(), signature=("memory",))
    ...
    try:
        with TestClient(app, raise_server_exceptions=False) as client:
            for _ in range(2):
                response = client.post("/api/v1/auth/login/local", data={"username": "user@example.com", "password": "wrong"})
...
def test_local_auth_throttle_config_validation():
    ...
    with pytest.raises(pydantic.ValidationError):
        LocalAuthConfig(account_max_attempts=0)
    with pytest.raises(pydantic.ValidationError):
        # 1 would mean a single typo locks an account, and before ge=2 it
        # disabled lockout entirely (the first-failure record expired
        # immediately and every failure re-created it).
        LocalAuthConfig(account_max_attempts=1)
    LocalAuthConfig(account_max_attempts=2)  # strictest legal value
    with pytest.raises(pydantic.ValidationError):
        LocalAuthConfig(account_lockout_seconds=0)
    with pytest.raises(pydantic.ValidationError):
        LocalAuthConfig(account_lockout_seconds=float("inf"))
