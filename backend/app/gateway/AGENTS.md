...
PATs (`Bearer dfp_...`) act as owner, never a service; invalid Bearers are hard 401s without cookie fallback. `PAT_ROUTE_SCOPE_RULES` default-denies routes by current owner permissions. UUID4 IDs are public refs; rows/audits are tenant-bound and token-free. `TrustedRunContextV1` v4 binds evidence. Required admission/control audit fails closed; bounded observations are best-effort. Revoked PATs cannot reuse evidence. Management/audit is session-only. See [the contract](../../../docs/AUDITABLE_AUTOMATION_IDENTITIES.md).
