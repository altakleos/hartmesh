...
- Provisioner-created Kubernetes sandbox Pods take their optional RuntimeClass
  ...
  to the sandbox, init containers, and sidecars. Image-specific non-root IDs
  remain an operator qualification concern. Sandbox volume mode is resolved
  once at provisioner startup: explicit `SANDBOX_VOLUME_MODE=pvc` requires both
  PVC names, explicit `hostpath` keeps the legacy layout, and inference accepts
  only both claim names set or both unset.
