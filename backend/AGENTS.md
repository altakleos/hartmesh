...
1. External platform -> Channel impl -> `MessageBus.publish_inbound()`
   - For signed GitHub, the webhook router verifies HMAC and registry bindings, then commits one bounded `inbound_receipts` row per matching binding in a single transaction before returning success. Only receipt IDs are published to `MessageBus`; the bus is a wake-up path, not the acknowledged copy. A storage failure returns a retryable response. Explicit unverified development webhooks and native sources without receipt support remain best-effort.
